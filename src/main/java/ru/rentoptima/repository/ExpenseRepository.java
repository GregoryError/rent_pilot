...
public interface ExpenseRepository extends JpaRepository<Expense, Long> {
    ...
    @Modifying
    @Query("DELETE FROM Expense e WHERE e.tenantId = :tenantId AND e.propertyId = :propertyId AND e.autoGenerated = true")
    int deleteAutoGeneratedByTenantAndProperty(Long tenantId, Long propertyId);

    @org.springframework.data.jpa.repository.Query(
            "SELECT COALESCE(SUM(e.amount), 0) FROM Expense e " +
                    "WHERE e.tenantId = :tenantId " +
                    "AND e.date BETWEEN :from AND :to " +
                    "AND e.categoryId <> :excludeCategoryId")
    java.math.BigDecimal sumExcludingCategory(
            ...
            @org.springframework.data.repository.query.Param("excludeCategoryId") Long excludeCategoryId);
}
