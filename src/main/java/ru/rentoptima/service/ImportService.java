...
public class ImportService {
        ...
        try (InputStream is = file.getInputStream();
            ...
            for (int i = 3; i <= sheet.getLastRowNum(); i++) {
                ...
                try {
                    ...
                    // Skip deleted and cancelled bookings
                    String mappedStatus = mapStatus(status);
                    if (!isActiveStatus(mappedStatus)) {
                        skipped++;
                        continue;
                    }
    ...
    /** Clear all imported bookings and auto-generated expenses for a property */
    @Transactional
    public ClearResult clearImportedData(Long tenantId, Long propertyId) {
        long deletedExpenses = expenseRepo.deleteAutoGeneratedByTenantAndProperty(tenantId, propertyId);
        long deletedBookings = bookingRepo.deleteByTenantIdAndPropertyId(tenantId, propertyId);
        log.info("Cleared data for property {}: {} bookings, {} expenses", propertyId, deletedBookings, deletedExpenses);
        return new ClearResult(deletedBookings, deletedExpenses);
    }
